...
## CP-20 · Phase 2b messaging + ratings — FOUR real bugs found (2026-07-23)
Building the web chat/ratings UI (samewayweb) meant driving the booking
lifecycle end-to-end for the first time on a real database. Four
production-breaking bugs surfaced, none of which any prior test caught,
all latent only because no ride had ever been completed for real.

BUG 1 (mig 017) — conversation trigger never ran. The INSERT used
  `case when new.seat_count > 1 then 'group' else 'booking' end`
with no ::conversation_type_t cast. In INSERT..VALUES the CASE's two
unknown literals resolve to `text` first and there is no implicit
text→enum cast, so it raised. Because the trigger fired on the same
UPDATE respond_booking performs, the exception aborted the whole RPC.

BUG 2 (mig 017) — trigger was AFTER UPDATE only. create_booking writes
instant-book bookings straight to status='confirmed' on INSERT, so even
with bug 1 fixed no conversation would ever exist for the instant-book
path — the path we actively promote. Now fires on INSERT OR UPDATE,
branches on TG_OP (OLD is unassigned during INSERT), is idempotent, and
adds the lead passenger explicitly (create_booking does not always write
a booking_passengers row for a solo booking, which would have left the
rider out of their own conversation). Includes a backfill.

BUG 3 (mig 018) — same enum-cast defect on notifications.type in
respond_booking() and respond_parcel(). respond_booking raised every
time: **a driver could not approve OR decline any booking at all.**
accept_booking_invite had the same shape in 002 but migration 011 had
already rewritten it and dropped that insert, so it needed nothing —
checked before touching it, to avoid reverting 011's integrity fix.
THE RULE, now hit three times: assignment context (UPDATE..SET) coerces
unknown literals to the enum; INSERT..VALUES does not. Cast explicitly.

BUG 4 (mig 019) — ride completion was impossible. trg_gamify_ride_
completed() assigned `freezes_used` TWICE in one ON CONFLICT DO UPDATE
SET (once to reset the quarterly allowance, once to spend a freeze), in
both the route_streaks and rider_streaks upserts. Postgres rejects that
outright. Since it fires on the same UPDATE as trg_ride_completed, **no
ride could ever be marked completed** — so bookings never closed, review
prompts never fired, ratings were unreachable, driver ride_count never
moved. Fixed by hoisting the quarter rollover into its own UPDATE ahead
of each upsert (also more correct: the freeze arithmetic now reads the
post-reset allowance). Migration generated by surgically transforming
the exact live function text rather than retyping ~80 lines, to
eliminate drift risk.

BUG 5 (mig 020) — participants lost their own ride. rides_select was
`status in ('published','full') or driver_id = auth.uid()`, which never
covered "I booked this ride." The moment a ride left published/full,
passengers lost read access to the ride row while their booking row
survived pointing at it: blank routes in My Bookings and in the messages
inbox/thread, and — worst — the ratings page 404'd for riders, making
the entire post-ride ratings loop unreachable from the passenger side.
NOT caught by the reviews RLS tests, which correctly assert a
non-participant cannot insert a review; the hole was upstream of that.
Added SECURITY DEFINER is_ride_participant(uuid) (driver, lead
passenger, group passenger, or member of a conversation on the ride —
the last also covers 016 inquiry threads) + supporting indexes.
Verified widening is exact: rider sees their completed ride, an
unrelated driver does not, anon still sees only published/full.

Verification: full local replica (PG16 + all 20 migrations + real
PostgREST 14.15 + prod-faithful auth.uid() + minted JWTs). Proven:
approve AND decline paths work; instant-book and approval paths both
create a 2-member conversation; ride completes → 2 bookings closed, 3
review prompts, ride_count 112→113; review inserts under RLS, rating
rollup 47→48; duplicate review rejected by unique constraint; contact
scrub turns a phone + email into ■■■ at write time; a non-member reads
[] for the conversation and its messages and is refused on POST; and
forging sender_id as another user is refused by RLS.

## Remaining after CP-20
1. Deploy: supabase db push (migrations 017-020). These fix currently
   BROKEN production behaviour — approval, completion, ratings.
2. The RLS test suite should gain cases for: driver approves/declines,
   ride completes, participant reads own completed ride. All four bugs
   were invisible to the existing 28 assertions.
3. Everything still open from CP-9 through CP-19 (plate-visibility flag,
   hosted project has no published rides, SMS OTP live check).
